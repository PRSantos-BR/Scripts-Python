...
def valida_alerta(url: str, end_point: str, token: str, numero_telefone: str) -> str:
    ...
    data_app = {"data": {"customer": {"networkMsisdn": numero_telefone}}}
...
def valida_cpf_custom_score(url: str, end_point: str, token: str, numero_cpf: str) -> str:
    ...
    numero_cpf = '{0}.{1}.{2}-{3}'.format(numero_cpf[:3], numero_cpf[3:6], numero_cpf[6:9], numero_cpf[9:])
    if resp.status_code == 200:
        valor_score = str(json.loads(resp.text)['score'][cliente['nome_score']]).rjust(6, chr(32))
        return 'Número CPF: {0} / Score ({1}): {2}'.format(numero_cpf, 
                                                           [tipo_score for tipo_score in json.loads(resp.text)['score'].keys()][0],
                                                           valor_score)
    else:
...
def valida_cpf_score(url: str, end_point: str, token: str, numero_cpf: str) -> str:
    ...
    data_app = {'data':{'customer': {'idCpf': numero_cpf}}}
    
    resp = requests.post(url + end_point,
                         headers=headers_api,
                         data=json.dumps(data_app))

    numero_cpf = data_app['data']['customer']['idCpf']
    numero_cpf = '{0}.{1}.{2}-{3}'.format(numero_cpf[:3], numero_cpf[3:6], numero_cpf[6:9], numero_cpf[9:])
    if resp.status_code == 200:
        valor_score = str(json.loads(resp.text)['data']['statsResult']['score']).rjust(4, chr(32))
...
def valida_endereco(url: str, end_point: str, token: str, numero_cep: str, numero_endereco: str, numero_cpf: str, numero_telefone: str) -> str:
    headers_api = {'Content-Type': 'application/json',
                   'X-CustomerID': 'Fulano de Tal',
                   'cache-control':'no-cache',
                   'user-agent':'curl/7.60.0',
    ...
    data_app = {"data": {"customer": {"addressZipCode": numero_cep, "addressNumber": numero_endereco, "cpf": numero_cpf, "networkMsisdn": numero_telefone}}}
...
def valida_telefone(url: str, end_point: str, token: str, numero_telefone: str, numero_cpf: str) -> str:
    ...
    data_app = {"data": {"customer": {"networkMsisdn": numero_telefone , "cpf": numero_cpf}}}
    ...
    if resp.status_code == 200:
        return 'Número Linha: {0} / Número CPF: {1} / {2} / {3}'.format(numero_telefone,
                                                                        numero_cpf,
                                                                        json.loads(resp.text)['data']['result']['summaryMessage'],
                                                                        json.loads(resp.text)['data']['result']['detailedMessage'])
    else:
...
def recupera_amostra_teste() -> list:
    # Valida Telefone 
    amostra = [('98984277777', '05475777777'), ('98984264210', '05475723362'), 
               ('79988248431', '03762479593'), ('85991156225', '01031691308'),
               ('85994238421', '02957764393'), ('84994926041', '10337020809'), ('85992169709', '93534213300'),
               ('87992045538', '10291828418'), ('85992273022', '02965776346'), ('81994150994', '08006549435')]

    # Valida Alerta
    """
    amostra = [('98984264210', '05475723362'), ('79988248431', '03762479593'), ('85991156225', '01031691308'),
               ('85994238421', '02957764393'), ('84994926041', '10337020809'), ('85992169709', '93534213300'),
               ('87992045538', '10291828418'), ('85992273022', '02965776346'), ('81994150994', '08006549435')]
    """
    ...
    """
    amostra = [('00000000000', ), ('08373632506', ), ('08373644342', ),
               ('08373646558', ), ('08373649735', ), ('08373651551', ), 
               ('08373651632', ), ('08373652604', ), ('08373653333', )]
    """
...
if respToken.status_code == 200:
    ...
    amostra = recupera_amostra_teste()
    # amostra = recupera_amostra()
